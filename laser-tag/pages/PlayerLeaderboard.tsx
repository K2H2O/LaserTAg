...
const PlayerLeaderboard = () => {
  ...
  const { gameCode, players: playersQuery } = router.query;
  ...
  useEffect(() => {
    ...
    if (typeof playersQuery === "string") {
      try {
        const parsedPlayers = JSON.parse(playersQuery);
        if (Array.isArray(parsedPlayers)) {
          setPlayers(parsedPlayers);
        } else {
          throw new Error("Parsed players is not an array");
        }
      } catch (error) {
      ...
      // Use dummy data if no players in query
      const dummyPlayers = [
        { username: 'player1', color: 'red', points: 50, hitsGiven: 5, hitsTaken: 2 },
        { username: 'player2', color: 'blue', points: 75, hitsGiven: 8, hitsTaken: 3 },
        { username: 'player3', color: 'green', points: 30, hitsGiven: 3, hitsTaken: 5 },
